...
@Composable
fun SenSettingsHelpScreen(onNavigateBack: () -> Unit) {
    ...
    val allRules = remember {
        listOf(
            ...
            RuleItem(HelpCategory.CVN, "Bỏ dấu sắc đuôi C, P, T", "c, p, t", "các ➔ cac, úp ➔ up, hát ➔ hat"),

            // 2. Quy tắc Y = I & UY = Y
            RuleItem(HelpCategory.CVN, "Thay Y = I & UY = Y", "Y=I, UY=Y", "y tá ➔ i tá, thúy ➔ thú", "AY, ÂY giữ nguyên (mây bay = mây bay)"),
            ...
            RuleItem(HelpCategory.ToneMarks, "Dấu Sắc (Không dấu phụ)", "Sắc = J", "vó ➔ voj", "Chữ có phụ âm cuối C, P, T thì bỏ J (khác ➔ kac, áp ➔ ap, phút ➔ fut)"),
            ...
            RuleItem(HelpCategory.PWord, "Chữ đệm câm P", "Suffix P", "long ➔ logp (tránh hiểu lầm với lỗ = log)", "P đặt sau vần rút gọn ở thanh ngang không dấu phụ: ag, ah, aj, eg, el, ev, ew, ez, ih, oah, og, oj, ol, ov, ow, oz, ug, yh"),
    ...
    SenSettingsScaffold(
        topBar = {
            SenSettingsTopBar(
                title = { SenSettingsTitle("Quy tắc gõ CVNSS 4.0") },
            ...
            LazyColumn(
                ...
                items(filteredRules) { ruleItem ->
                    SenSettingsMenu(
                        ...
                        headlineContent = {
                            SenSettingsLabel(ruleItem.title)
                        },
                        supportingContent = {
                            Column(modifier = Modifier.padding(top = 2.dp)) {
                                SenSettingsSupportingLabels("Ví dụ: ${ruleItem.example}")
...
fun SenSettingsHelpScreenPreview() {
    SenBoardTheme(dynamicColor = false) {
        SenSettingsHelpScreen(onNavigateBack = {})
    }
}
