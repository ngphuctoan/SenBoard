...
object CvnssEngine {
    ...
    private val reverse56 = mapOf(
        ...
        "yêt" to "id", "yêp" to "if", "yêc" to "is", "yên" to "il", "yêm" to "iv", "yêng" to "iz", "yêu" to "iw",
        "uôt" to "ud", "uôc" to "us", "uôn" to "ul", "uôm" to "uv", "uông" to "uz", "uôi" to "uj",
        "ươt" to "ud", "ươp" to "uf", "ươc" to "us", "ươn" to "ul", "ươm" to "uv", "ương" to "uz", "ươu" to "uw", "ươi" to "uj",
        "uât" to "ad", "uân" to "al", "uâng" to "az", "uây" to "aj",
        "uơt" to "od", "uơn" to "ol", "uơi" to "oj",
        "oăt" to "ad", "oăp" to "af", "oăc" to "as", "oăn" to "al", "oăm" to "av", "oăng" to "az",
    ...
    fun convertWord(rawWord: String): String {
        ...
        if (hasKhd) {
            when (lastChar) {
                ...
                'y' -> {
                    if (word == "y" || word == "ty" || word == "qy" || word.endsWith("ay") || word.endsWith("ây")) {
                        hasKhd = false
                    } else {
                        tone = ""; hatGroup = true
                    }
                ...
                'o' -> {
                    ...
                    val plainWords = setOf("cho", "do", "co", "to", "ho", "lo", "mo", "no", "po", "ro", "so", "bo", "go", "tho", "kho", "nho", "trho", "theo", "zo", "qo")
                    if (plainWords.contains(word) || stemBeforeO.isEmpty() || (word.endsWith("ao") && !word.endsWith("uao")) || word.endsWith("eo")) {
                        hasKhd = false
                    } else {
                        tone = ""; moonGroup = true
                    }
        ...
        if (hatGroup || moonGroup) {
            if (expandedVowel.endsWith("g") && !expandedVowel.endsWith("ng")) expandedVowel = expandedVowel.dropLast(1) + "ng"
            else if (expandedVowel.endsWith("h") && !expandedVowel.endsWith("nh")) expandedVowel = expandedVowel.dropLast(1) + "nh"
            else if (expandedVowel.endsWith("k") && !expandedVowel.endsWith("ch")) expandedVowel = expandedVowel.dropLast(1) + "ch"
        }
    ...
    fun encodeWord(cqnWord: String): String {
        ...
        if (rest.startsWith("ngh")) { initial = "w"; rest = rest.substring(3) }
        else if (rest.startsWith("ng")) { initial = "w"; rest = rest.substring(2) }
        else if (rest.startsWith("ph")) { initial = "f"; rest = rest.substring(2) }
        else if (rest.startsWith("qu")) { initial = "q"; rest = rest.substring(2) }
        else if (rest.startsWith("kh")) { initial = "k"; rest = rest.substring(2) }
        else if (rest.startsWith("gi")) { initial = "j"; rest = rest.substring(2) }
        else if (rest.startsWith("gh")) { initial = "g"; rest = rest.substring(2) }
        else if (rest.startsWith("nh") || rest.startsWith("ch") || rest.startsWith("th") || rest.startsWith("tr")) {
            val len = 2
            ...
            when (c0) {
                'đ' -> { initial = "d"; rest = rest.substring(1) }
                'd' -> { initial = "z"; rest = rest.substring(1) }
                'k' -> {
                ...
                'c' -> {
                    ...
                    if (!isVowel(c0)) { initial = c0.toString(); rest = rest.substring(1) }
                }
            }
        }
        ...
        shorthand = shorthand.replace('â', 'a').replace('ê', 'e').replace('ô', 'o')
                             .replace('ă', 'a').replace('ơ', 'o').replace('ư', 'u')
        ...
        if (hasHat) {
            ...
            khd = when (tone) {
                "sac" -> if (shorthand.endsWith("c") || shorthand.endsWith("p") || shorthand.endsWith("t")) "" else "j"
                "huyen" -> "l"
                "hoi" -> "z"
                "nga" -> "s"
                "nang" -> "r"
        ...
        if (cqnWord.firstOrNull()?.isUpperCase() == true) {
    ...
    private fun applyHatToVowel(vowel: String): String {
        ...
        if (v.contains("au")) {
            ...
            v = v.replace('a', 'â').replace('e', 'ê').replace('o', 'ô')
        }
        return v
    }
    ...
    private fun applyMoonToVowel(vowel: String): String {
        ...
        return v.replace('a', 'ă').replace('o', 'ơ')
    }
    ...
    private fun applyToneMark(word: String, tone: String): String {
        ...
        val normWord = java.text.Normalizer.normalize(word, java.text.Normalizer.Form.NFC)
