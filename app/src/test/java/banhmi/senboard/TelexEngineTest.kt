...
class TelexEngineTest {
    ...
    fun testTelexConversion() {
        ...
        // Vowel modifier cycling (aawa -> â, aawaw -> ă, oowo -> ô, oowow -> ơ, etc.)
        assertEquals("â", TelexEngine.convertWord("aawa"))
        assertEquals("ă", TelexEngine.convertWord("aawaw"))
        assertEquals("â", TelexEngine.convertWord("awa"))
        assertEquals("ă", TelexEngine.convertWord("awaw"))
        assertEquals("ấ", TelexEngine.convertWord("aawas"))
        assertEquals("nâm", TelexEngine.convertWord("naawam"))
        assertEquals("ô", TelexEngine.convertWord("oowo"))
        assertEquals("ơ", TelexEngine.convertWord("oowow"))
        assertEquals("ô", TelexEngine.convertWord("owo"))
        assertEquals("ơ", TelexEngine.convertWord("owow"))
        assertEquals("ố", TelexEngine.convertWord("oowos"))
        assertEquals("hôm", TelexEngine.convertWord("hoowom"))

        // Repeated letter escapes on cycled modifier vowels (owooo -> ooo, owoo -> oo, aawaa -> aa, aawaaa -> aaa)
        assertEquals("oo", TelexEngine.convertWord("owoo"))
        assertEquals("ooo", TelexEngine.convertWord("owooo"))
        assertEquals("oo", TelexEngine.convertWord("oowoo"))
        assertEquals("ooo", TelexEngine.convertWord("oowooo"))
        assertEquals("aa", TelexEngine.convertWord("awaa"))
        assertEquals("aaa", TelexEngine.convertWord("awaaa"))
        assertEquals("aa", TelexEngine.convertWord("aawaa"))
        assertEquals("aaa", TelexEngine.convertWord("aawaaa"))
        assertEquals("booo", TelexEngine.convertWord("bowooo"))
        assertEquals("baaa", TelexEngine.convertWord("baawaaa"))

        // Sequential tone key overwriting (aawf -> ằ, aawfr -> ẳ, aawfrj -> ặ, aawfrjs -> ắ)
        assertEquals("ằ", TelexEngine.convertWord("aawf"))
        assertEquals("ẳ", TelexEngine.convertWord("aawfr"))
        assertEquals("ặ", TelexEngine.convertWord("aawfrj"))
        assertEquals("ắ", TelexEngine.convertWord("aawfrjs"))
        assertEquals("ở", TelexEngine.convertWord("oowfr"))
        assertEquals("ể", TelexEngine.convertWord("eefr"))
        assertEquals("à", TelexEngine.convertWord("asf"))
